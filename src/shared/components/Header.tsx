...
export default function Header({ variant = 'default' }: HeaderProps) {
  ...
  const handleMyHomeClick = () => {
    if (isLoggedIn) navigate('/myhome');
    else openModal('loginRequired');
  };

  const getNavIconColor = (nav: ActiveNav) =>
    isOverlay ? '#FFFFFF' : activeNav === nav ? 'var(--color-primary-600)' : '#404959';
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div
            ...
            onClick={openSearch}
          >
            <span className="body-03 flex-1 text-gray-400">도시나 키워드로 검색하기</span>
            <Icon size="md">
